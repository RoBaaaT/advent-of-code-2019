...
fn main() {
    ...
    for opcode in input.split(b',') {
        ...
        let op: usize = op_str.trim_end().parse().unwrap();
        tape.push(op);
    }
    ...
    println!("Part 1: {}", compute_output(&tape));

    for noun in 0..99 {
        for verb in 0..99 {       
            tape[1] = noun;
            tape[2] = verb;
            if compute_output(&tape) == 19690720 {
...
fn compute_output(memory: &[usize]) -> usize {
    ...
    loop {
        ...
        if opcode == 99 {
            ...
            let param1_address = tape[address + 1];
            let param2_address = tape[address + 2];
